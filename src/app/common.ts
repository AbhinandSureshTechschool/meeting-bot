...
export const joinMeetWithRetry = async (
  ...
  try {
    ...
    if (retryCount <= config.retryCount) {
      ...
      await joinMeetWithRetry(processor, bearerToken, url, name, teamId, timezone, userId, retryCount, eventId, botId,  branchName, batchName, recordingFileName, logger);
    } else {
      throw error;
    }
  }
...
export const processMeetingJoin = async (
  ...
  try {
    ...
    await joinMeetWithRetry(processor, bearerToken, url, name, teamId, timezone, userId, 0, eventId, botId, branchName, batchName, recordingFileName, logger);
